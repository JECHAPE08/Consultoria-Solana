...
async function agregarCita(cliente: string, fecha: string) {
  try {
    ...
    const consultoriaData = await pg.program.account.consultoria.fetch(consultoriaPda);
    const citaExistente = consultoriaData.citas.find(c => c.cliente === cliente && c.fecha === fecha);
...
async function editarCita(cliente: string, nuevaFecha: string) {
  try {
    ...
    // Verificar el cambio
    const consultoriaData = await pg.program.account.consultoria.fetch(consultoriaPda);
    const citaEditada = consultoriaData.citas.find(c => c.cliente === cliente);
    if (citaEditada) {
